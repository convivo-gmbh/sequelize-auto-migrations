        ...
        for (let column in attributes)
        {
            ...
            if(typeof attributes[column]['type'] === 'undefined')
            {
                if(!attributes[column]['seqType'])
                {
                    ...
                    if(!['Sequelize.ARRAY(Sequelize.INTEGER)', 'Sequelize.ARRAY(Sequelize.STRING)', 'Sequelize.UUID'].includes(attributes[column]['seqType'])) {
                        delete attributes[column];
                        continue;
                    }
                    if(['Sequelize.UUID'].includes(attributes[column]['seqType'])) {
